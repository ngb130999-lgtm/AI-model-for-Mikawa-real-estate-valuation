...
def load_assets():
    ...
    parts = ["models/model_part_aa", 
             "models/model_part_ab", 
             "models/model_part_ac"]
    combined_model_path = "mikawa_stacking_model_combined.joblib"
    if not os.path.exists(combined_model_path):
        with open(combined_model_path, "wb") as outfile:
            for part in parts:
                with open(part, "rb") as infile:
                    outfile.write(infile.read())
    model = joblib.load(combined_model_path)
    encoder = joblib.load("models/mikawa_target_encoder.joblib")
    kmeans = joblib.load("models/mikawa_kmeans_cluster.joblib")
